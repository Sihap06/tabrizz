...
<div class="modal fade" id="hapusModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="modal-title">Hapus Rekap Penjualan</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<form name="" action="{{url('hapus_rekap')}}" method="POST" enctype="multipart/form-data">
				@csrf
				<div class="modal-body">
					<input type="hidden" name="id" id="id" value="">
					<p>
						Apakah anda yakin menghapus rekap penjualan ini ?
					</p>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
		...
		function load_data(from_date = '', to_date = '')
		{
			$('#dataTable').DataTable({
				footerCallback: function (row, data, start, end, display) {
					...
					// Update footer
					$(api.column(3).footer()).html(pageTotalStok);
					$(api.column(4).footer()).html('Rp'+ pageTotalHarga.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","));
				},
				processing:true,
				searching:true,
				info:false,
